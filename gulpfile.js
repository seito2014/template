...
gulp.task('watch-pc', function () {
    ...
    gulp.watch(configPath.pc.script.src2,['copy-pc']);
    gulp.watch(configPath.pc.image.src,['image-pc']);
});

gulp.task('default', ['build-pc','server-pc','watch-pc'], function() {});

gulp.task('release-pc', function() {
    return runSequence('pret-pc','style-min-pc','script-min-pc','image-min-pc', 'copy-min-pc');
});
...
gulp.task('watch-sp', function () {
    ...
    gulp.watch(configPath.sp.script.src2,['copy-sp']);
...
gulp.task('release-sp', function() {
    return runSequence('pret-sp','style-min-sp','script-min-sp','image-min-sp', 'copy-min-sp');
});